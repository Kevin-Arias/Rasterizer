...
private:
  ...
  struct SampleBuffer {
    ...
    void fill_color(int i, int j, Color c) {
      ...
      unsigned char red = (unsigned char) (c.r * 255);
      unsigned char green = (unsigned char) (c.g * 255);
      unsigned char blue = (unsigned char) (c.b * 255);
      p = {red, green, blue};
      return;
    }
    ...
    Color get_pixel_color() {
      ...
      for (int i = 0; i < samples_per_side; i += 1) {
        for (int j = 0; j < samples_per_side; j += 1) {
          Color c = Color(sub_pixels[i][j].data());
          total_red += (c.r * 255);
          total_green += (c.g * 255);
          total_blue += (c.b * 255);
//          PixelColorStorage &p = sub_pixels[i][j];
//          total_red += (float) p[0];
//          total_green += (float) p[1];
//          total_blue += (float) p[2];

        }
      }

      float average_red = (total_red/(samples_per_side*samples_per_side))/255.0f;
      float average_green = (total_green/(samples_per_side*samples_per_side))/255.0f;
      float average_blue = (total_blue/(samples_per_side*samples_per_side))/255.0f;
      Color final;
      final.r = average_red;
      final.g = average_green;
      final.b = average_blue;
      return final;
//      //Have to convert back to color [0 ... 1]
    }
