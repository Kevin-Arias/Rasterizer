...
float linetest(float temp_point_x, float temp_point_y, float first_point_x, float first_point_y, float second_point_x, float second_point_y) {
  return -(temp_point_x - first_point_x) * (second_point_y - first_point_y) + (temp_point_y - first_point_y) * (second_point_x - first_point_x);
}
...
//  int minx = min({(int)floor(x0), (int)floor(x1), (int)floor(x2)});
//  int maxx = max({(int) ceil(x0), (int) ceil(x1), (int) ceil(x2)});
//  int miny = min({(int) floor(y0), (int) floor(y1), (int) floor(y2)});
//  int maxy = max({(int) ceil(y0), (int) ceil(y1), (int) ceil(y2)});
//
//  float orientation = (y1 - y0) * (x2 - x1) - (x1 - x0) * (y2 - y1);
//  float firstx;
//  float firsty;
//  float secondx;
//  float secondy;
...
//    for (int column = minx; column < maxx; column += 1) {
//      float tempx = (float) column + (float) 0.5;
//      float tempy = (float) row + (float) 0.5;
//
//      //First line test
//      float first_line = linetest(tempx, tempy, firstx, firsty, secondx, secondy);
//      //Second line test
//      float second_line = linetest(tempx, tempy, secondx, secondy, thirdx, thirdy);
//      //Third line test
//      float third_line = linetest(tempx, tempy, thirdx, thirdy, firstx, firsty);
//
...
//  for (int row = miny; row < maxy; row += 1) {
//    for (int column = minx; column < maxx; column += 1) {
//      float units = 1.0f / (float)(sqrt(sample_rate) * 2);
//      float current_row = units;
//      float current_column = units;
//      for (int sub_row = 0; sub_row < (sqrt(sample_rate)); sub_row += 1) {
//        for (int sub_column = 0; sub_column < (sqrt(sample_rate)); sub_column += 1) {
//          float tempx = column + current_column;
//          float tempy = row + current_row;
//          //First line test
//          float first_line = linetest(tempx, tempy, firstx, firsty, secondx, secondy);
//          //Second line test
//          float second_line = linetest(tempx, tempy, secondx, secondy, thirdx, thirdy);
...
//          if (first_line >= 0 && second_line >= 0 && third_line >= 0) {
//            if (tri != NULL) {
//              float alpha = (-(tempx-secondx)*(thirdy-secondy) + (tempy-secondy)*(thirdx-secondx))/(-(firstx-secondx)*(thirdy-secondy)+(firsty-secondy)*(thirdx-secondx));
//              float beta = (-(tempx-thirdx)*(firsty-thirdy) + (tempy-thirdy)*(firstx-thirdx))/(-(secondx-thirdx)*(firsty-thirdy)+(secondy-thirdy)*(firstx-thirdx));
//              float gamma = 1-alpha-beta;
//              Vector3D vector = Vector3D((double)alpha, (double)beta, (double)gamma);
//              samplebuffer[row][column].fill_color(sub_row, sub_column,tri->color(vector));
//            } else {
//              samplebuffer[row][column].fill_color(sub_row, sub_column,color);
//              //samplebuffer[row][column].fill_pixel(color);
//            }
...
//          }
//
//          current_column = current_column + (2.0f * units);
//        }
//        current_column = units;
//        current_row = current_row + (2.0f * units);
//      }
//    }
//  }
//
...
//              float gamma = 1-alpha-beta;
//              Vector3D vector = Vector3D((double)alpha, (double)beta, (double)gamma);
//              Vector3D temp_dx = Vector3D(0,0,0);
//              Vector3D temp_dy = Vector3D(0,0,0);
//              SampleParams sp;
//              sp.lsm = lsm;
//              sp.psm = psm;
//              samplebuffer[row][column].fill_color(sub_row, sub_column,tri->color(vector,temp_dx, temp_dy,sp));
//            } else {
//              samplebuffer[row][column].fill_color(sub_row, sub_column,color);
//              //samplebuffer[row][column].fill_pixel(color);
//            }
              ...
              for (int sub_column = 0; sub_column < (sqrt(sample_rate)); sub_column += 1) {
                  ...
                  if (first_line >= 0 && second_line >= 0 && third_line >= 0) {
                      if (tri != NULL) {
                          ...
                          float alphadx = (-((tempx+1)-secondx)*(thirdy-secondy) + (tempy-secondy)*(thirdx-secondx))/(-(firstx-secondx)*(thirdy-secondy)+(firsty-secondy)*(thirdx-secondx));
                          float betadx = (-((tempx+1)-thirdx)*(firsty-thirdy) + (tempy-thirdy)*(firstx-thirdx))/(-(secondx-thirdx)*(firsty-thirdy)+(secondy-thirdy)*(firstx-thirdx));
                          float gammadx = 1-alphadx-betadx;

                          float alphady = (-(tempx-secondx)*(thirdy-secondy) + ((tempy+1)-secondy)*(thirdx-secondx))/(-(firstx-secondx)*(thirdy-secondy)+(firsty-secondy)*(thirdx-secondx));
                          float betady = (-(tempx-thirdx)*(firsty-thirdy) + ((tempy+1)-thirdy)*(firstx-thirdx))/(-(secondx-thirdx)*(firsty-thirdy)+(secondy-thirdy)*(firstx-thirdx));
                          float gammady = 1-alphady-betady;

                          Vector3D vector = Vector3D((double)alpha, (double)beta, (double)gamma);
                          Vector3D temp_dx = Vector3D((double) alphadx, (double)betadx, (double) gammadx);
                          Vector3D temp_dy = Vector3D((double)alphady,(double)betady,(double)gammady);
                          SampleParams sp;
                          sp.lsm = lsm;
                          sp.psm = psm;
                          if (alphadx < 0 or betadx < 0 or gammadx < 0 or alphady < 0 or betady < 0 or gammady < 0) {
                              sp.lsm = L_ZERO;
                          }
                          samplebuffer[row][column].fill_color(sub_row, sub_column,tri->color(vector,temp_dx, temp_dy,sp));
                      } else {
